...
void CSoundRender_Emitter::start(ref_sound* _owner, BOOL _loop, float delay)
{
	...
	// Reset fade config to defaults on every (re)start -- covers pooled/reused emitters.
	// A script that wants a custom fade must call set_fade_out/set_fade_in after play().
...
void CSoundRender_Emitter::set_fade_in(float duration_s, int curve)
{
	...
	{
		// Force silence now; update_culling() will ramp fade_volume back up
		// to 1.0 over fade_in_duration_s on subsequent frames.
		fade_in_elapsed = 0.f;
		fade_volume = 0.f;
	}
}
